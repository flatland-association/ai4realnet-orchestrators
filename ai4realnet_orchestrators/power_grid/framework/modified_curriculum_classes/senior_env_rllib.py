...
from edited_curr_agent_classes.utilities import find_best_line_to_reconnect, split_and_execute_action, revert_topo
...
class SeniorEnvRllib(gym.Env):
    ...
    def __init__(self, config: RLlibEnvConfig,
        ...
        """
        self.calls_to_step = 0
        self.calls_to_run_na = 0
    ...
    def run_next_action(self) -> np.ndarray:
        ...
        """
        self.calls_to_run_na += 1
        # init values
        continue_running = True
        cur_obs = self.single_env.get_obs()
        orig_obs = cur_obs.copy()
        if self.use_perturb_agent and self.perturb_agent != None:
            cur_obs = self.perturb_agent.perturb(cur_obs)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, Union[float, int], bool, bool, dict]:
        ...
        cur_obs, reward, self.done, self.info = split_and_execute_action(env=self.single_env,
                                                                         ...
                                                                         perturb_agent=self.perturb_agent if self.use_perturb_agent else None)

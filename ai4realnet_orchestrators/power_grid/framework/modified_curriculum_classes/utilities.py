...
def obs_to_graph(observation):
    """
    Converts a Grid2Op observation into a graph:
    - x: node features (tensor of shape [num_nodes, feature_dim])
    - edge_index: tensor of edges [2, num_edges]
    """
    # Example placeholder
    x = torch.tensor(observation.to_vect(), dtype=torch.float).unsqueeze(1)
...
def split_and_execute_action(env: BaseEnv, action_vect: np.ndarray, perturb_agent : Optional[BasePerturbationAgent] = None) -> Tuple[BaseObservation, float, bool, dict]:
    ...
    obs = env.get_obs()
    orig_obs = obs.copy()
    if perturb_agent != None:
        obs = perturb_agent.perturb(obs)
    info = {}
    for _ in range(len(split_actions)):
        # Iterate through remaining actions and choose the best one to execute next
        obs_min = np.inf
        ...
        for act in split_actions:
            ...
            obs_, _, done, _ = orig_obs.simulate(act_plus_reconnect)
    ...
    return orig_obs, cum_rew, done, info
